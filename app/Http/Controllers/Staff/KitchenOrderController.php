...
class KitchenOrderController extends Controller
{
    ...
    public function store(Request $request, StaffRoomOrderService $staffRoomOrderService)
    {
        ...
        $staffRoomOrderService->createManualOrder('kitchen', $data, $request->user());

        return back()->with('success', 'Kitchen order created for the room.');
    }

    public function updateStatus(Request $request, Order $order, KitchenInventoryService $inventory)
    {
        $this->authorizeAreaAccess();
        abort_unless($order->service_area === 'kitchen', 404);
    ...
    protected function authorizeAreaAccess(): void
    {
        ...
        if ($user->hasAnyRole(['md', 'manager', 'frontdesk', 'kitchen'])) {
            return;
        }
