<template>
  <ManagerLayout>
    <div class="space-y-6">
      <div class="flex justify-between items-center">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">
          Profit & Loss
        </h1>
        <div class="flex items-center space-x-4">
          <input
            type="date"
            v-model="filters.from"
            class="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            @change="submit"
          />
          <span class="text-gray-500">to</span>
          <input
            type="date"
            v-model="filters.to"
            class="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            @change="submit"
          />
          <button
            @click="exportReport"
            class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
          >
            Export PDF
          </button>
        </div>
      </div>

      <div class="bg-white dark:bg-gray-900 rounded-lg shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            Period: {{ report.period.from }} to {{ report.period.to }}
          </h2>
        </div>

        <div class="p-6 space-y-6">
          <!-- Revenue Section -->
          <div>
            <h3 class="text-lg font-semibold text-green-600 dark:text-green-400 mb-4">Revenue</h3>
            <div class="space-y-2">
              <div
                v-for="item in report.revenue"
                :key="item.account"
                class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-800"
              >
                <span class="text-gray-700 dark:text-gray-300">{{ item.account }}</span>
                <span class="font-medium text-green-600 dark:text-green-400">
                  ₦{{ formatNumber(item.amount) }}
                </span>
              </div>
            </div>
          </div>

          <!-- Expenses Section -->
          <div>
            <h3 class="text-lg font-semibold text-red-600 dark:text-red-400 mb-4">Expenses</h3>
            <div class="space-y-2">
              <div
                v-for="item in report.expenses"
                :key="item.account"
                class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-800"
              >
                <span class="text-gray-700 dark:text-gray-300">{{ item.account }}</span>
                <span class="font-medium text-red-600 dark:text-red-400">
                  ₦{{ formatNumber(item.amount) }}
                </span>
              </div>
            </div>
          </div>

          <!-- Totals Section -->
          <div class="mt-6 pt-6 border-t-2 border-gray-200 dark:border-gray-700">
            <div class="space-y-2">
              <div class="flex justify-between py-2">
                <span class="text-lg font-semibold text-gray-700 dark:text-gray-300">Total Revenue</span>
                <span class="text-lg font-semibold text-green-600 dark:text-green-400">
                  ₦{{ formatNumber(report.totals.revenue) }}
                </span>
              </div>
              <div class="flex justify-between py-2">
                <span class="text-lg font-semibold text-gray-700 dark:text-gray-300">Total Expenses</span>
                <span class="text-lg font-semibold text-red-600 dark:text-red-400">
                  ₦{{ formatNumber(report.totals.expenses) }}
                </span>
              </div>
              <div class="flex justify-between py-3 text-xl font-bold border-t-2 border-gray-300 dark:border-gray-600">
                <span class="text-gray-900 dark:text-gray-100">Net Profit</span>
                <span :class="report.totals.net_profit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
                  ₦{{ formatNumber(report.totals.net_profit) }}
                </span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </ManagerLayout>
</template>

<script setup>
import ManagerLayout from '@/Layouts/Staff/ManagerLayout.vue'
import { useForm, router } from '@inertiajs/vue3'

const props = defineProps({
  report: Object,
  filters: Object,
})

const filters = useForm({
  from: props.filters?.from || '',
  to: props.filters?.to || '',
})

const submit = () => {
  filters.get(route('admin.reports.profit-loss'), {
    preserveState: true,
  })
}

const exportReport = () => {
  window.open(`/admin/reports/profit-loss/export/pdf?from=${filters.from}&to=${filters.to}`, '_blank')
}

const formatNumber = (num) => {
  return new Intl.NumberFormat('en-NG').format(Math.abs(num))
}
</script>