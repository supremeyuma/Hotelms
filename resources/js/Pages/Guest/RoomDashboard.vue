            ...
            <div v-if="billHistory && billHistory.length > 0" class="space-y-3 max-h-[50vh] overflow-y-auto pr-2">
              <div v-for="charge in billHistory" :key="charge.id" class="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <div class="space-y-0.5">
                  ...
                  <p class="text-[10px] text-slate-600 font-bold uppercase tracking-tighter">{{ new Date(charge.created_at).toLocaleString() }}</p>
                </div>
                <p class="font-black text-slate-900">₦{{ Number(charge.amount).toLocaleString() }}</p>
              </div>
            </div>
            <div v-else class="py-10 text-center text-slate-600 italic">No transactions found.</div>
          </div>
        </template>
      </Modal>
        ...
        <template #title>
          <div class="flex items-center gap-2">
            ...
            <div>
              ...
              <p class="text-xs font-semibold text-slate-600 uppercase tracking-wide">Kitchen & Bar</p>
            </div>
          </div>
        </template>
            ...
            <div v-if="orders && orders.length > 0" class="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              ...
              >
                <div class="px-5 py-4 flex justify-between items-center border-b border-white/50">
                  <div>
                    <p class="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em]">Code - {{order.service_area}}</p>
                ...
                <div class="px-5 py-4 space-y-2">
                  <div v-for="item in order.items" :key="item.id" class="flex justify-between items-center">
                    <div class="flex items-center gap-2">
                      <span class="w-5 h-5 flex items-center justify-center bg-white rounded-md text-[10px] font-black text-slate-600 border border-slate-100">
                        {{ item.qty }}
                      </span>
                      <span class="text-sm font-bold text-slate-600">{{ item.item_name }}</span>
                    </div>
                    <span class="text-xs font-bold text-slate-600">₦{{ Number(item.price * item.qty).toLocaleString() }}</span>
                  </div>
                </div>
      ...
      <Modal :show="showMaintenanceModal" @close="closeMaintenanceModal">
        ...
        <template #content>
          <form @submit.prevent="submitMaintenance" class="space-y-5">
            <div>
              <label class="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2 block">Issue Category</label>
            ...
            <div>
              <label class="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2 block">Description</label>
            ...
            <div>
              <label class="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2 block">Attach Photo</label>
      ...
      <Modal :show="showExtendStayModal" @close="closeExtendStayModal">
        ...
        <template #content>
          <form @submit.prevent="submitExtendStay" class="space-y-5">
            <div>
              <label class="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2 block">New Checkout Date</label>
